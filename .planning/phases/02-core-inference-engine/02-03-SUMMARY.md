...
decisions:
  ...
  - "dev-models and cuda features added to prunr-core/Cargo.toml to propagate feature flags cleanly"
metrics:
...
Implemented `OrtEngine` struct in `crates/prunr-core/src/engine.rs`:
...
- `dev-models` feature added to `prunr-core/Cargo.toml` propagating `prunr-models/dev-models`

### Task 2: pipeline.rs process_image() (commit de6834f)

Implemented `process_image()` in `crates/prunr-core/src/pipeline.rs`:
...
- **Files modified:** crates/prunr-core/src/pipeline.rs

**7. [Rule 2 - Missing critical functionality] dev-models and cuda features missing from prunr-core**
- **Found during:** Task 1 compilation
- **Issue:** `#[cfg(feature = "dev-models")]` in prunr-core referenced a feature not declared in its Cargo.toml, causing `unexpected_cfg` warnings and incorrect conditional compilation
- **Fix:** Added `dev-models = ["prunr-models/dev-models"]` and `cuda = []` to prunr-core features
- **Files modified:** crates/prunr-core/Cargo.toml
...
| Check | Status |
|-------|--------|
| crates/prunr-core/src/engine.rs exists | FOUND |
| crates/prunr-core/src/pipeline.rs exists | FOUND |
