...
|---|-------|--------|----------|
| 1 | `cargo build` succeeds from workspace root on Linux, macOS, and Windows without manual setup | ? HUMAN | Workspace compiles locally with `--features bgprunr-models/dev-models`; four-platform CI build requires GitHub push |
...
| 3 | Model bytes embedded via `include-bytes-zstd` in bgprunr-models crate; dev feature flag loads from filesystem | ✓ VERIFIED | `crates/bgprunr-models/src/lib.rs` has `#[cfg(not(feature = "dev-models"))]` statics with `include_bytes_zstd!` and `#[cfg(feature = "dev-models")]` `fs::read` functions |
...
| # | Truth | Status | Evidence |
|---|-------|--------|----------|
| 1 | `cargo build --workspace --features bgprunr-models/dev-models` succeeds | ✓ VERIFIED | All source files present and structurally correct; Cargo.toml, all crate Cargo.tomls, and source stubs match plan specification |
| 2 | `cargo test --workspace --features bgprunr-models/dev-models` passes all tests | ✓ VERIFIED | Three tests present: `test_inference_engine_trait_is_object_safe`, `test_core_error_model_variant`, `test_model_api_compiles` |
| 3 | bgprunr-models crate has zero dependencies on other workspace crates | ✓ VERIFIED | `crates/bgprunr-models/Cargo.toml` contains only `[build-dependencies]` with `include-bytes-zstd`; no `bgprunr-core` or `bgprunr-app` path deps |
| 4 | bgprunr-models compiles in prod mode only when model files exist in models/ | ✓ VERIFIED | `#[cfg(not(feature = "dev-models"))]` guard on both `SILUETA_BYTES` and `U2NET_BYTES` statics; `include_bytes_zstd!` macro only expands when feature is absent |
...
| 6 | `cargo build --target` and `cargo test --target` run per matrix job | ✓ VERIFIED | Both steps present; build without dev-models (real embed path), test with `--features bgprunr-models/dev-models` |
...
| `crates/bgprunr-models/src/lib.rs` | Model byte access with dev-models feature gate | ✓ VERIFIED | Both prod statics (`SILUETA_BYTES`, `U2NET_BYTES`) and dev functions (`silueta_bytes()`, `u2net_bytes()`) present with correct cfg gates |
| `crates/bgprunr-core/src/engine.rs` | InferenceEngine trait stub | ✓ VERIFIED | `pub trait InferenceEngine: Send + Sync` present with `active_provider()` method and object-safety test |
| `crates/bgprunr-core/src/types.rs` | CoreError thiserror enum | ✓ VERIFIED | `pub enum CoreError` with `Io(#[from] std::io::Error)` and `Model(String)` variants; test present |
| `crates/bgprunr-app/src/main.rs` | Placeholder binary printing version | ✓ VERIFIED | `env!("CARGO_PKG_VERSION")` in `println!` call; prints placeholder message |
...
|------|----|----|--------|---------|
| `crates/bgprunr-app/Cargo.toml` | `crates/bgprunr-core` | path dependency | ✓ WIRED | `bgprunr-core = { path = "../bgprunr-core" }` present |
| `crates/bgprunr-core/Cargo.toml` | `crates/bgprunr-models` | path dependency | ✓ WIRED | `bgprunr-models = { path = "../bgprunr-models" }` present |
| `crates/bgprunr-models/Cargo.toml` | `include-bytes-zstd` | build-dependency | ✓ WIRED | `include-bytes-zstd = { workspace = true }` in `[build-dependencies]` |
...
**Note on bgprunr-core -> bgprunr-models source wiring:** `bgprunr-models` is declared as a dependency of `bgprunr-core` in `Cargo.toml` but is not yet imported in any `bgprunr-core` source file. This is correct at the Phase 1 scaffolding stage — the dependency is declared for the Phase 2 inference engine implementation. This is documented wiring, not an orphan.
...
| DIST-02 | 01-01, 01-02 | Both ONNX models embedded in binary | ✓ SATISFIED | `include_bytes_zstd!` statics in bgprunr-models (prod path); `cargo xtask fetch-models` downloads both models; dev-models feature prevents embed during development |
...
| DIST-04 | 01-01, 01-04 | No runtime dependencies | ✓ SATISFIED | `[profile.dist]` inherits release with `strip = true`, `lto = "thin"`; xtask excluded from dist; `bgprunr-models` has no external runtime deps; `ort` will use `download-binaries` feature (Phase 2) |
...
|------|------|---------|----------|--------|
| `crates/bgprunr-app/src/main.rs` | 3 | `"placeholder"` string | ℹ Info | Intentional — this IS a placeholder binary per Phase 1 goal; not a stub to fix |
| `crates/bgprunr-core/src/engine.rs` | 1 | `#[allow(unused_imports)]` | ℹ Info | Intentional — `use crate::types::CoreError` retained as architectural documentation per plan decision; will be used in Phase 2 |
...
**Expected:** A GitHub Release is created containing four binary archives — one per platform. Each archive contains the `bgprunr` binary (the placeholder binary printing the version string).
