...
requires: []
provides:
  - Cargo workspace with four members: bgprunr-core, bgprunr-models, bgprunr-app, xtask
  - InferenceEngine trait stub (Send+Sync) in bgprunr-core
  - CoreError thiserror enum with Io and Model variants in bgprunr-core
  - dev-models feature gate in bgprunr-models with fs-read functions
  - Production zstd model embedding guards (cfg not dev-models) in bgprunr-models
  - Placeholder binary printing CARGO_PKG_VERSION in bgprunr-app
...
tech-stack:
  ...
  patterns:
    ...
    - bgprunr-models isolation: zero deps on other workspace crates, model recompile is isolated
...
key-files:
  created:
    ...
    - crates/bgprunr-core/src/engine.rs (InferenceEngine trait)
    - crates/bgprunr-core/src/types.rs (CoreError enum)
    - crates/bgprunr-models/Cargo.toml (dev-models feature, include-bytes-zstd build dep)
    - crates/bgprunr-models/src/lib.rs (model embedding with cfg gates)
    - crates/bgprunr-app/Cargo.toml
    - crates/bgprunr-app/src/main.rs (placeholder binary)
    - xtask/Cargo.toml
...
patterns-established:
  ...
  - "Model isolation pattern: bgprunr-models has zero path deps to other workspace crates; dependency arrow is app -> core -> models (never reverse)"
...
**Cargo workspace skeleton with InferenceEngine/CoreError trait stubs in bgprunr-core, dev-models feature gate in bgprunr-models, and placeholder binary in bgprunr-app — all four packages build and test clean**
...
- Four-crate Cargo workspace builds clean with `cargo build --workspace --features bgprunr-models/dev-models`
- Three tests pass: `test_inference_engine_trait_is_object_safe`, `test_core_error_model_variant`, `test_model_api_compiles`
- bgprunr-models crate is fully isolated (zero workspace crate deps) so model recompilation is independent of source changes
...
- `crates/bgprunr-core/Cargo.toml` — depends on bgprunr-models and thiserror
- `crates/bgprunr-core/src/lib.rs` — exports InferenceEngine and CoreError
- `crates/bgprunr-core/src/engine.rs` — InferenceEngine trait (Send+Sync) with mock test
- `crates/bgprunr-core/src/types.rs` — CoreError enum with Io and Model variants + test
- `crates/bgprunr-models/Cargo.toml` — dev-models feature, include-bytes-zstd build dep only
- `crates/bgprunr-models/src/lib.rs` — cfg-gated model statics (prod) and fs-read fns (dev) + API test
- `crates/bgprunr-app/Cargo.toml` — depends on bgprunr-core, defines bgprunr binary
- `crates/bgprunr-app/src/main.rs` — placeholder printing CARGO_PKG_VERSION
...
- **Verification:** `cargo build --workspace --features bgprunr-models/dev-models` completes with no warnings
...
- bgprunr-models isolation confirmed; model recompilation will not cascade to core/app on source changes
