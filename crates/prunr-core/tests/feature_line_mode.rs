...
use prunr_core::{
    compose_subject_outline, process_image_from_decoded, ComposeMode, EdgeEngine, EdgeScale,
    EdgeSettings, InputTransform, LineStyle, MaskSettings, ModelKind, OrtEngine, ProgressStage,
};
...
fn line_mode_off_runs_seg_only() {
    if skip_if_no_ort("line_mode_off") {
        return;
    }
    let engine =
...
fn line_mode_edges_only_produces_lines_from_source() {
    if skip_if_no_ort("line_mode_edges_only") {
        return;
    }
...
fn line_mode_subject_outline_combines_seg_and_edge() {
    if skip_if_no_ort("line_mode_subject_outline") {
        return;
    }
    let seg_engine =
        OrtEngine::new_cpu_only(ModelKind::Silueta, 1).expect("OrtEngine::new_cpu_only(Silueta)");
    ...
    let edge_res = edge_engine
        .infer_all_tensors(&img)
        .expect("infer_all_tensors");
    let edge = default_edge_settings(
        EdgeScale::Fused,
        ComposeMode::SubjectFilled,
        LineStyle::Solid,
    );
    ...
    assert!(
        max_a > 200,
        "SubjectOutline must keep some opaque subject pixels, got max α={max_a}"
    );
}
