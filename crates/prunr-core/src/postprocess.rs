...
fn apply_edge_shift(mask: &mut GrayImage, shift: f32) {
    ...
    let step = |src: &[u8], dst: &mut [u8]| {
        let process_row = |(y, row): (usize, &mut [u8])| {
            ...
            if y > 0 && y < h - 1 && w > 2 {
                let row_mid = y * w;
                let row_prev = row_mid - w;
                let row_next = row_mid + w;

                if erode {
                    // Left edge (x=0): 3x3 window clamped to [0, w-1] uses columns 0 and 1.
                    let mut m = src[row_prev].min(src[row_prev + 1]);
                    m = m.min(src[row_mid]).min(src[row_mid + 1]);
                    m = m.min(src[row_next]).min(src[row_next + 1]);
                    row[0] = m;

                    // Interior: full 3x3 window fits without clamping.
                    for x in 1..w - 1 {
                        let mut m = src[row_prev + x - 1];
                        m = m.min(src[row_prev + x]);
                        m = m.min(src[row_prev + x + 1]);
                        m = m.min(src[row_mid + x - 1]);
                        m = m.min(src[row_mid + x]);
                        m = m.min(src[row_mid + x + 1]);
                        m = m.min(src[row_next + x - 1]);
                        m = m.min(src[row_next + x]);
                        m = m.min(src[row_next + x + 1]);
                        row[x] = m;
                    }

                    // Right edge (x=w-1): 3x3 window clamped uses columns w-2 and w-1.
                    let x = w - 1;
                    let mut m = src[row_prev + x - 1].min(src[row_prev + x]);
                    m = m.min(src[row_mid + x - 1]).min(src[row_mid + x]);
                    m = m.min(src[row_next + x - 1]).min(src[row_next + x]);
                    row[x] = m;
                } else {
                    // Left edge (x=0)
                    let mut m = src[row_prev].max(src[row_prev + 1]);
                    m = m.max(src[row_mid]).max(src[row_mid + 1]);
                    m = m.max(src[row_next]).max(src[row_next + 1]);
                    row[0] = m;

                    // Interior
                    for x in 1..w - 1 {
                        let mut m = src[row_prev + x - 1];
                        m = m.max(src[row_prev + x]);
                        m = m.max(src[row_prev + x + 1]);
                        m = m.max(src[row_mid + x - 1]);
                        m = m.max(src[row_mid + x]);
                        m = m.max(src[row_mid + x + 1]);
                        m = m.max(src[row_next + x - 1]);
                        m = m.max(src[row_next + x]);
                        m = m.max(src[row_next + x + 1]);
                        row[x] = m;
                    }
                    ...
                    // Right edge (x=w-1)
                    let x = w - 1;
                    let mut m = src[row_prev + x - 1].max(src[row_prev + x]);
                    m = m.max(src[row_mid + x - 1]).max(src[row_mid + x]);
                    m = m.max(src[row_next + x - 1]).max(src[row_next + x]);
                    row[x] = m;
                }
            } else {
                // Top/bottom rows or narrow images: use the slow path with boundary clamping.
