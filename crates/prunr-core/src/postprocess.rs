...
fn apply_edge_shift(mask: &mut GrayImage, shift: f32) {
    ...
    let step = |src: &[u8], dst: &mut [u8]| {
        let process_row = |(y, row): (usize, &mut [u8])| {
            let row_off = y * w;
            if y == 0 || y == h - 1 || w < 3 {
                // Boundary or narrow image: use slow path with clamping
                let yi = y as i32;
                ...
                // Interior row: handle x=0, 1..w-1, w-1 separately to skip clamping
                // and hoist the erode conditional out of the inner loop.
                ...
                // x = 0
                {
                    ...
                    for &off in &[prev_off, row_off, next_off] {
                        let v0 = src[off];
                        let v1 = src[off + 1];
                        if erode {
                            e = e.min(v0).min(v1);
                ...
                // x = 1..w-1 (Interior fast-path)
                if erode {
                    for x in 1..w - 1 {
                        let mut e = src[prev_off + x - 1];
                        e = e.min(src[prev_off + x]);
                        e = e.min(src[prev_off + x + 1]);
                        e = e.min(src[row_off + x - 1]);
                        e = e.min(src[row_off + x]);
                        e = e.min(src[row_off + x + 1]);
                        e = e.min(src[next_off + x - 1]);
                        e = e.min(src[next_off + x]);
                        e = e.min(src[next_off + x + 1]);
                        row[x] = e;
                    }
                } else {
                    for x in 1..w - 1 {
                        let mut e = src[prev_off + x - 1];
                        e = e.max(src[prev_off + x]);
                        e = e.max(src[prev_off + x + 1]);
                        e = e.max(src[row_off + x - 1]);
                        e = e.max(src[row_off + x]);
                        e = e.max(src[row_off + x + 1]);
                        e = e.max(src[next_off + x - 1]);
                        e = e.max(src[next_off + x]);
                        e = e.max(src[next_off + x + 1]);
                        row[x] = e;
                    }
                }
