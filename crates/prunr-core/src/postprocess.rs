...
fn apply_edge_shift(mask: &mut GrayImage, shift: f32) {
    ...
    let step = |src: &[u8], dst: &mut [u8]| {
        // Hoist the erode/dilate branch out of the inner loops.
        if erode {
            let process_row = |(y, row): (usize, &mut [u8])| {
                let row_off = y * w;
                // Interior fast-path: avoids boundary clamping and branches.
                if y > 0 && y < h - 1 && w > 2 {
                    let p = row_off - w;
                    let c = row_off;
                    let n = row_off + w;
                    ...
                    // Interior: fully unrolled 3x3 min.
                    for x in 1..w - 1 {
                        row[x] = src[p + x - 1]
                            .min(src[p + x])
                            .min(src[p + x + 1])
                    ...
                    // Fully slow path for top/bottom rows (boundary clamping required).
                    let yi = y as i32;
                    for x in 0..w {
                        let xi = x as i32;
                        let mut m = 255u8;
...
mod tests {
    ...
    fn apply_edge_shift_1k_bench() {
        ...
        let mut mask = GrayImage::from_fn(w, h, |x, y| image::Luma([(x ^ y) as u8]));

        bench_report(
            &format!("apply_edge_shift_1k_bench ({w}x{h})"),
            2,
            5,
