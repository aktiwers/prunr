            ...
            hardware_intent = egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| match app.settings_tab {
                    ...
                    SettingsTab::Behavior => {
                        render_tab_behavior(ui, &mut app.settings);
                        None
                    }
                    SettingsTab::Hotkeys => {
                        render_tab_hotkeys(ui);
                        None
                    }
                })
                .inner;
...
fn dispatch_hardware_intent(app: &mut PrunrApp, intent: HardwareSectionIntent) {
    use crate::runtime_install::{start_install, InstallEvent};
    match intent {
...
fn render_hardware_section(
    ...
    egui::Grid::new("hardware_grid")
        ...
        .show(ui, |ui| {
            kv_row(
                ui,
                "CPU",
                &format!("{} ({})", p.cpu_vendor, p.cpu_brand),
    ...
    ui.horizontal(|ui| {
        ui.label(
            RichText::new("Compiled-model cache")
                .color(theme::TEXT_PRIMARY)
                .size(theme::FONT_SIZE_BODY),
        );
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
...
fn render_tab_general(
    ...
    ui.horizontal(|ui| {
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ...
            ui.label(
                RichText::new(format!("{}", settings.parallel_jobs))
                    .color(theme::TEXT_PRIMARY)
                    .size(theme::FONT_SIZE_BODY)
                    .strong(),
            );
    ...
    if has_gpu {
        ui.checkbox(
            &mut settings.force_cpu,
            RichText::new("Force CPU this session")
                .color(theme::TEXT_PRIMARY)
                .size(theme::FONT_SIZE_BODY),
        );
        hint(
            ui,
            "Resets to GPU on next launch. Useful for debugging GPU misbehaviour.",
        );
        ui.add_space(theme::SPACE_MD);
    }

    ui.checkbox(
        &mut settings.live_preview,
    ...
    hint(
        ui,
        "Auto-rerun mask and edge tweaks while you drag sliders.",
    );
    intent
}
...
fn render_tab_behavior(ui: &mut egui::Ui, settings: &mut Settings) {
    ...
    ui.checkbox(
        &mut settings.auto_process_on_import,
    ...
    hint(
        ui,
        "Use dark tones for the transparency pattern (helps on light results).",
    );
    ui.add_space(theme::SPACE_SM);

    ui.checkbox(
        &mut settings.auto_hide_adjustments,
    ...
    hint(
        ui,
        "Collapse the toolbar when the cursor leaves it. Toggle with Shift+H.",
    );
    ui.add_space(theme::SPACE_MD);

    section_heading(ui, "Stack passes (chain mode)");
    ui.checkbox(
        &mut settings.chain_mode,
        RichText::new("Use last result as input")
            .color(theme::TEXT_PRIMARY)
            .size(theme::FONT_SIZE_BODY),
    );
    hint(
        ui,
        "Process feeds on the previous output instead of the original — stack effects.",
    );
    ...
    ui.checkbox(
        &mut settings.export_split_layers,
    ...
    hint(
        ui,
        "New images inherit this preset. The reset button on the toolbar restores its values.",
    );
}
...
fn render_tab_hotkeys(ui: &mut egui::Ui) {
    ...
    ui.add_space(theme::SPACE_MD);
    ui.label(
        RichText::new("Rebinding will land in a future release.")
            .color(theme::TEXT_HINT)
            .size(theme::FONT_SIZE_MONO),
    );
}
