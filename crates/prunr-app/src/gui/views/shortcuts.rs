...
pub fn render(ctx: &egui::Context) -> bool {
    theme::standard_modal_window(
        ...
        "Keyboard Shortcuts",
        [
            theme::SHORTCUT_OVERLAY_WIDTH,
            theme::SHORTCUT_OVERLAY_HEIGHT,
        ],
        |ui| {
            ui.vertical(|ui| {
...
pub fn render_shortcut_grid(ui: &mut egui::Ui) {
    ...
    egui::Grid::new("shortcuts_grid")
        ...
        .show(ui, |ui| {
            let row = |ui: &mut egui::Ui, k: &str, a: &str| kv_row(ui, k, a, theme::TEXT_PRIMARY);
            ...
            row(
                ui,
                &format!("{modifier}+Z"),
                "Undo last action (stroke, result, or preset)",
            );
            row(
                ui,
                &format!("{modifier}+Shift+Z / {modifier}+Y"),
