...
impl RuntimeId {
    fn pypi_package(self) -> &'static str {
        match self {
            Self::OpenVino => "onnxruntime-openvino",
        }
    }

    fn pypi_version(self) -> &'static str {
        match self {
            Self::OpenVino => prunr_runtime_install::PINNED_ORT_VERSION,
        }
    }
    ...
    pub fn display_name(self) -> &'static str {
        match self {
            Self::OpenVino => "OpenVINO Runtime",
        }
    }
    ...
    pub fn first_launch_prompt_body(self) -> &'static str {
        match self {
            Self::OpenVino => {
                "We detected hardware that can use OpenVINO Runtime. \
    ...
    pub fn install_button_label(self) -> &'static str {
        match self {
            Self::OpenVino => "Install OpenVINO Runtime",
        }
    }
...
fn run_install(
    ...
    let mut hooks = ri::DownloadHooks {
...
fn query_pypi(rt: RuntimeId) -> Result<ri::WheelInfo, String> {
    ...
    let metadata: serde_json::Value = resp.json().map_err(|e| format!("PyPI JSON parse: {e}"))?;
...
fn prepare_and_extract(bytes: &[u8], rt: RuntimeId) -> Result<PathBuf, String> {
    ...
    if target_dir.exists() {
        std::fs::remove_dir_all(&target_dir).map_err(|e| format!("clean install dir: {e}"))?;
    }
    std::fs::create_dir_all(&target_dir).map_err(|e| format!("create install dir: {e}"))?;
